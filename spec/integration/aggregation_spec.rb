...
describe "aggregation" do
  ...
  context "filtering" do
    it "returns just one filtered terms aggregation" do
      response = Product.search "*", filters: { category: "Book" }, aggregations: :category
    ...
    it "returns just one filtered stats aggregation" do
      response = Product.search "*", filters: { category: "Vesture" }, aggregations: { available_stock: { type: "stats" } }
      available_stock_stats = response.aggregations["available_stock"]

      expect(available_stock_stats["count"]).to eq(1)
      expect(available_stock_stats["min"]).to eq(600)
      expect(available_stock_stats["max"]).to eq(600)
      expect(available_stock_stats["sum"]).to eq(600)
      expect(available_stock_stats["avg"]).to eq(600)
    end
  end
